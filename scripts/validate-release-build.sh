...
reject_file "scripts/release/install-node.sh"
reject_file "scripts/release/install.ps1"
reject_file "scripts/release/install.cmd"
reject_file "scripts/release/installers"
reject_file "scripts/release/installers/install-linux.sh"
reject_file "scripts/release/installers/install-unix-common.sh"
reject_file "scripts/release/installers/install-macos.sh"
reject_file "scripts/release/installers/install-windows.ps1"
need_file "README.md"
...
need_grep '^POOL_TEMPLATE_TTL_REFRESH_MS=1000$' ".env.example"
need_grep 'POOL_TEMPLATE_TTL_REFRESH_MS: \$\{POOL_TEMPLATE_TTL_REFRESH_MS:-1000\}' "docker-compose.yml"
...
need_grep '^BDAG_WATCHDOG_PAUSE_POOL_DURING_NODE_RESTART=auto$' ".env.example"
need_grep '^BDAG_WATCHDOG_PAUSE_POOL_DURING_NODE_RESTART=auto$' "ops/config/stack-defaults.env"
need_grep 'BDAG_WATCHDOG_PAUSE_POOL_DURING_NODE_RESTART: \$\{BDAG_WATCHDOG_PAUSE_POOL_DURING_NODE_RESTART:-auto\}' "docker-compose.yml"
need_grep 'pool-self-gates-node-health' "ops/watchdog.py"
...
need_grep 'BDAG_WATCHDOG_NODE_PEER_LEAD_HARD_STALL_JOB_AGE_SECONDS: \$\{BDAG_WATCHDOG_NODE_PEER_LEAD_HARD_STALL_JOB_AGE_SECONDS:-90\}' "docker-compose.yml"
need_grep '^BDAG_WATCHDOG_NODE_TEMPLATE_SYNC_WEDGE_CONFIRM_SECONDS=300$' ".env.example"
...
need_grep 'BDAG_NODEWORKER_MINING_READINESS_GRACE=20m' "ops/config/stack-defaults.env"
need_grep '--health.mining-readiness-timeout=\${BDAG_NODEWORKER_MINING_READINESS_TIMEOUT:-30m}' "docker/entrypoint-nodeworker.sh"
need_grep '--health.mining-readiness-grace=\${BDAG_NODEWORKER_MINING_READINESS_GRACE:-20m}' "docker/entrypoint-nodeworker.sh"
...
need_grep 'release-payload.env' "scripts/release/install.sh"
need_grep 'set_env_value .env DOCKER_PLATFORM "\$DOCKER_PLATFORM"' "scripts/release/install.sh"
need_grep 'BDAG_SNAPSHOT_URL' "scripts/release/install.sh"

reject_grep 'amd64 emulation' "scripts/release/install.sh"
reject_grep 'BDAG_RELEASE_INSTALL_CHAIN_DB_ONLY' "scripts/release/install.sh"
reject_grep 'chain-db-only' "scripts/release/install.sh"
