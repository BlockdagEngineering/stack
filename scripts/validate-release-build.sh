...
reject_file() {
  local file="$1"
  [[ ! -e "$root/$file" ]] || fail "$file must not exist"
}
...
reject_grep 'DASHBOARD_REPO=|DASHBOARD_REF=' ".env.example"
reject_grep 'DASHBOARD_REPO|DASHBOARD_REF' "docker-compose.yml"
reject_grep 'DASHBOARD_REPO|DASHBOARD_REF|git clone.*dashboard' "dockerfile"
need_grep 'dashboard_src: \$\{DASHBOARD_SRC_CONTEXT:-\.\./dashboard\}' "docker-compose.yml"
need_grep 'COPY --from=dashboard_src \. /opt/dashboard' "dockerfile"
...
reject_file "scripts/export-snapshot.sh"
reject_file "scripts/join-snapshot-parts.sh"
reject_file "scripts/ci-assemble-snapshot.sh"
reject_file "ops/hourly-chain-snapshot.sh"
reject_file "ops/chain-presync.sh"
reject_file "ops/chain-snapshot-common.sh"
reject_file "ops/systemd/user-bdag-hourly-snapshot.service"
reject_file "ops/systemd/user-bdag-hourly-snapshot.timer"
reject_file "ops/systemd/user-bdag-chain-presync.service"
reject_file "ops/systemd/user-bdag-chain-presync.timer"
reject_grep 'latest\.bdsnap|SNAPSHOT_PATH|BDAG_SNAPSHOT|BDAG_REQUIRE_SNAPSHOT' "scripts/release/installers/install-unix-common.sh"
reject_grep 'latest\.bdsnap|SNAPSHOT_PATH|BDAG_SNAPSHOT|BDAG_REQUIRE_SNAPSHOT' "scripts/release/installers/install-windows.ps1"
