...
need_grep 'POOL_ALLOW_MULTIPLE_BLOCK_CANDIDATES_PER_JOB=false' ".env.example"
...
need_grep 'POOL_ALLOW_MULTIPLE_BLOCK_CANDIDATES_PER_JOB=false' "ops/build-pi5-arm64-release.sh"
...
need_grep 'BDAG_FASTSYNC_PEER_ORDERING=latency' ".env.example"
need_grep 'BDAG_FASTSYNC_PEER_ORDERING=latency' "ops/build-pi5-arm64-release.sh"
need_grep 'BDAG_FASTSYNC_LAN_PEERS' "ops/build-pi5-arm64-release.sh"
need_grep 'latency-first FastSync candidates enabled' "ops/build-pi5-arm64-release.sh"
need_grep 'latency-first FastSync candidates enabled' "docker/entrypoint-nodeworker.sh"
reject_grep 'private/VPN second, public last' ".env.example"
reject_grep 'private/VPN second, public last' "ops/build-pi5-arm64-release.sh"
if [[ "$mode" == "source" ]]; then
  need_grep 'BDAG_FASTSYNC_PEER_ORDERING=latency' ".env.cpu.example"
fi
