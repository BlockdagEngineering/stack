...
need_grep 'BDAG_ASIC_LAN_INTERFACE=' ".env.example"
...
need_grep 'BDAG_ASIC_LAN_INTERFACE: .*BDAG_ASIC_LAN_INTERFACE' "docker-compose.yml"
...
need_grep 'ensure_asic_lan_address "\$lan_ip" "\$asic_lan_interface"' "ops/release-install.sh"
need_grep 'BDAG_ASIC_LAN_INTERFACE so the host can own' "ops/release-install.sh"
need_grep 'ensure_link_local_asic_pool_address' "ops/apply-mining-host-tuning.sh"
need_grep 'ip addr replace "\$pool_host/16" dev "\$asic_iface" scope link' "ops/apply-mining-host-tuning.sh"
need_grep 'configured_env_value BDAG_ASIC_LAN_INTERFACE' "scripts/local-deploy-test-release.sh"
need_grep 'BDAG_POOL_HOST=\$current_pool_host is link-local' "scripts/local-deploy-test-release.sh"
...
if [[ "$mode" == "source" ]]; then
  ...
  need_grep 'ensure_asic_lan_address "\$POOL_LAN_IP" "\$ASIC_LAN_INTERFACE"' "scripts/release/installers/install-unix-common.sh"
