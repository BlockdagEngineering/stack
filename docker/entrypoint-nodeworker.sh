...
append_unique_peer() {
  ...
  case "$ORDERED_BOOTSTRAP_SEEN" in
    *"|$peer|"*) return 0 ;;
  esac
  bucket+=("$peer")
  ORDERED_BOOTSTRAP_SEEN="${ORDERED_BOOTSTRAP_SEEN}|$peer|"
}
...
ordered_bootstrap_peers() {
  local node_args="$1"
  local ordering="${BDAG_BOOTSTRAP_PEER_ORDERING:-p2p-latency}"
  ...
  case "$ordering" in
    ...
    *) log "unknown BDAG_BOOTSTRAP_PEER_ORDERING=$ordering; using p2p-latency" ;;
  esac
  generic_peers="${BOOTSTRAP_PEER_ADDRESSES:-} $config_peers $(addpeer_values "$node_args" | paste -sd, - || true)"
