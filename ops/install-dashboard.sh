...
Environment=BDAG_PROJECT_ROOT=$PROJECT_ROOT
Environment=BDAG_RUNTIME_DIR=$RUNTIME_DIR
Environment=BDAG_SENTINEL_USER_SERVICES=${INSTANCE}-dashboard.service,${INSTANCE}-watchdog.service,${INSTANCE}-p2p-guard.service
Environment=BDAG_SENTINEL_USER_TIMERS=${INSTANCE}-stack-sentinel.timer,${INSTANCE}-sync-coordinator.timer,${INSTANCE}-chain-restore-guard.timer,${INSTANCE}-chain-presync.timer,${INSTANCE}-hourly-snapshot.timer,${INSTANCE}-local-peers.timer
EnvironmentFile=-$ENV_FILE
