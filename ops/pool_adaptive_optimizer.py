...
DEFAULT_SAFE_TEMPLATE_TTL_MS = 500
DEFAULT_SAFE_BLOCK_CANDIDATE_JOB_AGE_MS = 800
DEFAULT_SAFE_VARDIFF_TARGET_SHARE_SECONDS = 3.0
DEFAULT_SAFE_VARDIFF_WINDOW_SECONDS = 120
MIN_TEMPLATE_TTL_MS = 250
MAX_TEMPLATE_TTL_MS = 1000
MIN_BLOCK_CANDIDATE_JOB_AGE_MS = 600
MAX_BLOCK_CANDIDATE_JOB_AGE_MS = 1400
MIN_TARGET_SHARE_SECONDS = 2.0
MAX_TARGET_SHARE_SECONDS = 6.0
DEFAULT_WINDOW_SECONDS = 900
DEFAULT_SAMPLE_INTERVAL_SECONDS = 30
DEFAULT_CHANGE_COOLDOWN_SECONDS = 1800
...
def dashboard_job_health_ok(status: dict[str, Any]) -> bool | None:
    candidates: list[Any] = [
        status.get("source_job_health"),
        ((status.get("pool_metrics") or {}).get("source_job_health") if isinstance(status.get("pool_metrics"), dict) else None),
        (
            ((status.get("pool_metrics") or {}).get("template_backend_state") or {}).get("job_health")
            if isinstance(status.get("pool_metrics"), dict) and isinstance((status.get("pool_metrics") or {}).get("template_backend_state"), dict)
            else None
        ),
    ]
    for candidate in candidates:
        if isinstance(candidate, dict) and isinstance(candidate.get("ok"), bool):
            return bool(candidate.get("ok"))
    return None
...
def summarize_safety(
    ...
    if abort_reason:
        warnings.append(f"runtime_abort_{reason_token(abort_reason)}")
    dashboard_health_ok = dashboard_job_health_ok(status)
    if float(metrics.get("pool_job_health_ok", 1.0) or 0.0) <= 0.0 and dashboard_health_ok is not True:
        if accepted_blocks_per_hour > 0 and status.get("can_submit_blocks") is not False:
            warnings.append("pool_job_health_not_ok_transient")
        else:
            violations.append("pool_job_health_not_ok")
    elif float(metrics.get("pool_job_health_ok", 1.0) or 0.0) <= 0.0 and dashboard_health_ok is True:
        warnings.append("pool_job_health_metric_lag")
    if status.get("overall") not in {"ok", None}:
        if status.get("overall") == "degraded" and status.get("can_submit_blocks") is not False:
            warnings.append("dashboard_overall_degraded")
        else:
    ...
    if pressure["iowait_percent"] is not None and pressure["iowait_percent"] >= args.high_iowait_percent:
        warnings.append("host_iowait_high")
    ...
    if share_accept_ratio is not None and share_accept_ratio < args.warn_share_accept_ratio:
        warnings.append("share_acceptance_weak")
    ...
    if block_rejects_per_hour > args.max_block_rejects_per_hour:
        if accepted_blocks_per_hour > 0 and status.get("can_submit_blocks") is not False:
            warnings.append("block_reject_rate_high")
        else:
...
def add_sample_health_signals(
    safety: dict[str, Any],
    status: dict[str, Any],
    args: argparse.Namespace,
    pool_job_health_bad_samples: int,
    pool_job_health_metric_lag_samples: int,
    stale_job_samples: int,
    invalidated_job_samples: int,
    sample_count: int,
) -> dict[str, Any]:
    health_bad_ratio = pool_job_health_bad_samples / max(1, sample_count)
    accepted_blocks_per_hour = float(safety.get("accepted_blocks_per_hour") or 0.0)
    if health_bad_ratio >= args.max_pool_job_health_bad_sample_ratio:
        if accepted_blocks_per_hour > 0 and status.get("can_submit_blocks") is not False:
            safety.setdefault("warnings", []).append("pool_job_health_bad_sample_ratio_high")
        else:
            safety.setdefault("violations", []).append("pool_job_health_bad_sample_ratio_high")
            safety["ok"] = False
    elif pool_job_health_bad_samples:
        safety.setdefault("warnings", []).append("pool_job_health_bad_samples")
    if pool_job_health_metric_lag_samples:
        safety.setdefault("warnings", []).append("pool_job_health_metric_lag_samples")
    if stale_job_samples:
        safety.setdefault("warnings", []).append("stale_current_job_samples")
    if invalidated_job_samples:
        safety.setdefault("warnings", []).append("invalidated_current_job_samples")
    safety["sample_health"] = {
        "samples": sample_count,
        "pool_job_health_bad_samples": pool_job_health_bad_samples,
        "pool_job_health_metric_lag_samples": pool_job_health_metric_lag_samples,
        "pool_job_health_bad_sample_ratio": round(health_bad_ratio, 6),
        "stale_job_samples": stale_job_samples,
        "invalidated_job_samples": invalidated_job_samples,
    }
    safety["ok"] = not safety.get("violations")
    return safety
...
def choose_next_config(current: AdaptiveConfig, summary: dict[str, Any], status: dict[str, Any], args: argparse.Namespace) -> tuple[AdaptiveConfig, str]:
    ...
    stale_block_candidates = float(rejected_by_reason.get("stale_block_candidate", 0.0))
    old_template_age = sum(float(value) for key, value in block_reasons.items() if "old-template-age" in key)
    local_stale_blocks = sum(
        float(value)
        for key, value in block_reasons.items()
        if "stale-job" in key or "stale-parent" in key or "stale_block_candidate" in key
    )
    fetch_avg = templates.get("fetch_avg_seconds")

    if pressure["iowait_percent"] is not None and pressure["iowait_percent"] >= args.high_iowait_percent:
        next_share = clamp_float(current.vardiff_target_share_seconds + args.vardiff_step_seconds, MIN_TARGET_SHARE_SECONDS, MAX_TARGET_SHARE_SECONDS)
        next_ttl = clamp_int(current.template_ttl_ms + args.template_step_ms, MIN_TEMPLATE_TTL_MS, MAX_TEMPLATE_TTL_MS)
        return (
    ...
    if old_template_age > 0 or local_stale_blocks > 0 or stale_block_candidates > 0:
        next_age = clamp_int(current.block_candidate_job_age_ms - args.block_age_step_ms, MIN_BLOCK_CANDIDATE_JOB_AGE_MS, MAX_BLOCK_CANDIDATE_JOB_AGE_MS)
        return (
            AdaptiveConfig(current.template_ttl_ms, next_age, current.vardiff_target_share_seconds, current.vardiff_window_seconds),
            "stale block-candidate evidence is high; lower local candidate age cliff one damped step",
        )
...
def adaptive_abort_reason(status: dict[str, Any]) -> str:
    if status.get("can_mine") is False:
        return "dashboard says can_mine=false"
    if status.get("can_submit_blocks") is False:
        return "dashboard says can_submit_blocks=false"
    if status.get("overall") not in {"ok", "degraded", None}:
        return f"dashboard overall={status.get('overall')}"
    return ""


def cooldown_active(state: dict[str, Any], cooldown_seconds: float) -> tuple[bool, float | None]:
    if cooldown_seconds <= 0:
        return False, None
    last_change = parse_utc_seconds(state.get("last_applied_change_at"))
...
def smooth_score(state: dict[str, Any], score: float, alpha: float) -> float:
    alpha = clamp_float(alpha, 0.0, 1.0)
    previous = float_or_none(state.get("smoothed_score"))
    if previous is None:
        return score
    return round(previous + alpha * (score - previous), 6)
...
def build_report(run_dir: Path, payload: dict[str, Any]) -> Path:
    ...
    for row in payload.get("iterations", []):
        ...
        rows.append(
            ...
            f"<td>{html.escape(', '.join((safety.get('violations') or []) + (safety.get('warnings') or [])))}</td>"
            "</tr>"
        )
...
<body><main>
  ...
  <table>
    <thead><tr><th>#</th><th>Action</th><th>Reason</th><th>Score</th><th>Accepted blocks/h</th><th>Rejected blocks/h</th><th>Share accept</th><th>Signals</th></tr></thead>
...
def run_controller(args: argparse.Namespace) -> dict[str, Any]:
    ...
    while args.iterations <= 0 or iteration < args.iterations:
        ...
        while time.monotonic() - window_start < args.window_seconds:
            ...
            sample_count += 1
            metric_job_health_bad = float(current_metrics.get("pool_job_health_ok", 1.0) or 0.0) <= 0
            dashboard_health_ok = dashboard_job_health_ok(status)
            if metric_job_health_bad and dashboard_health_ok is not True:
                pool_job_health_bad_samples += 1
            elif metric_job_health_bad and dashboard_health_ok is True:
                pool_job_health_metric_lag_samples += 1
            if timing.numeric_delta(previous_sample_metrics, current_metrics, "pool_job_health_current_job_stale_miners") > 0:
                stale_job_samples += 1
            if timing.numeric_delta(previous_sample_metrics, current_metrics, "pool_job_health_current_job_invalidated_miners") > 0:
                invalidated_job_samples += 1
            previous_sample_metrics = current_metrics
            abort = adaptive_abort_reason(status)
            sample_log_every = max(1, int(args.sample_log_every or 1))
            if sample_count == 1 or sample_count % sample_log_every == 0 or abort:
                timing.append_jsonl(
                    events_path,
                    {
                        "time": timing.utc_iso(),
                        ...
                        "pool_job_health_bad_sample_ratio": round(pool_job_health_bad_samples / max(1, sample_count), 6),
                        "pool_job_health_metric_lag_samples": pool_job_health_metric_lag_samples,
                    },
                )
        ...
        safety = add_sample_health_signals(
            safety,
            status_after,
            args,
            pool_job_health_bad_samples,
            pool_job_health_metric_lag_samples,
            stale_job_samples,
            invalidated_job_samples,
            sample_count,
        )
        score = score_window(summary, status_after)
        smoothed = smooth_score(state, score, args.score_ewma_alpha)
        ...
        if not safety["ok"]:
            ...
            cooling, cooldown_age = cooldown_active(state, args.change_cooldown_seconds)
            if action == "apply" and cooling:
                action = "hold"
                next_config = current
                reason = f"damping cooldown active: last applied change was {round(cooldown_age or 0, 1)}s ago"

        decision = Decision(action=action, reason=reason, next_config=next_config, safety=safety, score=smoothed, mode=mode)
        ...
        state = {
            ...
            "last_applied_change_at": state.get("last_applied_change_at"),
            "last_decision": {
        ...
        if apply_enabled and action in {"apply", "revert"}:
            state["last_applied_change_at"] = state["updated_at"]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--max-iowait-percent", type=float, default=75.0)
    parser.add_argument("--high-iowait-percent", type=float, default=25.0)
    parser.add_argument("--max-pool-job-health-bad-sample-ratio", type=float, default=0.25)
