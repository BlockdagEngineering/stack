...
class WatchdogSyncRestartTests(unittest.TestCase):
    def test_active_import_requires_fresh_import_age_when_importing_flag_is_stuck(self) -> None:
        ...
        state = {
            "last_sync_height_changed_at_by_node": {"bdag-miner-node-1": now - 700},
        }

        with mock.patch.object(watchdog, "NODES", ["bdag-miner-node-1"]):
    ...
    def test_active_import_allows_fresh_importing_node(self) -> None:
        ...
        self.assertEqual(["bdag-miner-node-1"], active)
    ...
    def test_active_import_suppression_does_not_consume_repair_cooldown(self) -> None:
        ...
        with mock.patch.object(watchdog, "NODES", ["bdag-miner-node-1"]), mock.patch.object(
    ...
    def test_targeted_node_restart_uses_runtime_container_name(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir, mock.patch.object(
            ...
            ok = watchdog.run_node_restart("bdag-miner-node-1", "unit test")

        self.assertTrue(ok)
        self.assertEqual([["docker", "restart", "bdag-miner-node-1"]], commands)
    ...
    def test_node_dag_tip_cleanup_runs_narrow_cleanuptips_repair(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir, mock.patch.object(
            ...
            ok = watchdog.run_node_dag_tip_cleanup("bdag-miner-node-1", "unit test")
    ...
    def test_check_once_repairs_missing_dag_tip_before_generic_restart(self) -> None:
        ...
        status = {
            "failures": [
                "bdag-miner-node-1 wrapper is up but bdag child is not running",
                "bdag-miner-node-1 has critical log entries",
            ],
            "stack_failures": [
                "bdag-miner-node-1 wrapper is up but bdag child is not running",
                "bdag-miner-node-1 has critical log entries",
        ...
        self.assertEqual([("bdag-miner-node-1", cleanup_calls[0][1])], cleanup_calls)
        self.assertIn("--cleanuptips", cleanup_calls[0][1])
        self.assertEqual(0, result["watchdog_state"]["consecutive_failures"])
        self.assertEqual({"bdag-miner-node-1": now}, result["watchdog_state"]["last_node_dag_tip_cleanup_at_by_node"])
