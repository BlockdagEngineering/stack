...
class RpcRouterTransientTemplateTests(unittest.TestCase):
    def test_benign_initial_download_probe_does_not_switch_under_pool_pressure(self) -> None:
        status = {
            "nodes": {
                "bdag-miner-node-1": {
                    "child_running": True,
        ...
        self.assertEqual(0.0, decision["score_delta"])
        self.assertNotIn(
            "template-probe-transient-initial-download-1-1",
            decision["scores"]["bdag-miner-node-2"]["reasons"],
        )

    def test_hard_template_probe_error_can_switch_under_pool_pressure(self) -> None:
        status = {
            "nodes": {
                "bdag-miner-node-1": {
                    "child_running": True,
                ...
                "bdag-miner-node-2": {
                    ...
                    "template_probe_sample_count": 1,
                    "template_probe_error_count": 1,
                    "template_probe_benign_tx_template_error": False,
                },
            },
            "pool_health": {
                "block_submit_error_count": 10,
