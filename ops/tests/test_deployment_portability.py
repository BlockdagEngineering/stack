...
"""
    ...
    def test_release_build_validator_focuses_on_release_assets(self) -> None:
        validator = (ROOT_DIR / "scripts" / "validate-release-build.sh").read_text(encoding="utf-8")

        self.assertIn('need_grep \'target: linux-amd64\' ".github/workflows/build.yml"', validator)
        self.assertIn('need_grep \'target: linux-arm64\' ".github/workflows/build.yml"', validator)
        self.assertIn('need_grep \'scripts/render-release-bootstrap.py\' ".github/workflows/build.yml"', validator)
        self.assertIn('need_grep \'release-payload.env\' ".github/workflows/build.yml"', validator)
        self.assertIn('reject_grep \'build-pi5-arm64-release\\.sh\' ".github/workflows/build.yml"', validator)
        self.assertIn('reject_grep \'validate-pi5-restart-hardening\\.sh\' ".github/workflows/build.yml"', validator)
