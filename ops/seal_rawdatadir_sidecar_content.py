...
def signer_from_env(env: dict[str, str]) -> dict[str, str] | None:
    key_hex = (env.get("BDAG_RAWDATADIR_ARTIFACT_SIGNING_KEY_HEX") or "").strip()
    if not key_hex:
        return None
    ...
    if len(key_bytes) == 64:
        ...
        raise ValueError("BDAG_RAWDATADIR_ARTIFACT_SIGNING_KEY_HEX must be a 32-byte seed or 64-byte ed25519 private key")
    ...
    return {
        "key_id": env.get("BDAG_RAWDATADIR_ARTIFACT_SIGNING_KEY_ID") or "rawdatadir-sidecar",
