...
install_fastsnap_seed_timer() {
  ...
  if [[ ! -x "$ROOT/ops/build-fastsnap-seed.sh" || ! -x "$ROOT/ops/fastartifact_sidecar.py" || ! -f "$ROOT/ops/systemd/user-bdag-fastartifact-sidecar.service" || ! -f "$ROOT/ops/systemd/user-bdag-fastartifact-sidecar.timer" ]]; then
    warn "Fast Artifact sidecar files are missing under $ROOT/ops"
    return 0
  fi

  local user_systemd_dir="${XDG_CONFIG_HOME:-$HOME/.config}/systemd/user"
  local user_config_dir="${XDG_CONFIG_HOME:-$HOME/.config}"
  local pool_api_port node1_metrics_port node2_metrics_port max_lag require_fresh sidecar_lag sidecar_age node_image
  ...
  sidecar_lag="$(env_value BDAG_FASTARTIFACT_SIDECAR_MAX_SEED_LAG 10000)"
  sidecar_age="$(env_value BDAG_FASTARTIFACT_SIDECAR_MAX_ARCHIVE_AGE_SECONDS 7200)"
  node_image="$(env_value BDAG_FASTSNAP_NODE_IMAGE "$(env_value BLOCKDAG_NODE_IMAGE bdag-release/node:local)")"
...
BDAG_FASTSNAP_NODE_IMAGE=$node_image
...
EOF
  cat > "$user_config_dir/bdag-fastartifact-sidecar.env" <<EOF
# Generated by ops/install-p2p-services.sh. Override here for this host.
BDAG_PROJECT_ROOT=$ROOT
BDAG_ENV_FILE=$ROOT/.env
BDAG_COMPOSE_FILE=$ROOT/docker-compose.yml
...
BDAG_FASTSNAP_REQUIRE_EXPORT_BACKEND_FRESH=$require_fresh
BDAG_FASTSNAP_MAX_EXPORT_BACKEND_LAG=$max_lag
BDAG_FASTARTIFACT_SIDECAR_MAX_SEED_LAG=$sidecar_lag
BDAG_FASTARTIFACT_SIDECAR_MAX_ARCHIVE_AGE_SECONDS=$sidecar_age
BDAG_FASTSNAP_NODE_IMAGE=$node_image
BDAG_FASTSNAP_NODE_METRICS_URLS=node1=http://127.0.0.1:$node1_metrics_port/debug/metrics/prometheus,node2=http://127.0.0.1:$node2_metrics_port/debug/metrics/prometheus
EOF
  if [[ -f "$ROOT/ops/systemd/user-bdag-fastsnap-seed.service" && -f "$ROOT/ops/systemd/user-bdag-fastsnap-seed.timer" ]]; then
  ...
  install -m 0644 "$ROOT/ops/systemd/user-bdag-fastartifact-sidecar.service" "$user_systemd_dir/bdag-fastartifact-sidecar.service"
  install -m 0644 "$ROOT/ops/systemd/user-bdag-fastartifact-sidecar.timer" "$user_systemd_dir/bdag-fastartifact-sidecar.timer"
  systemctl --user daemon-reload
  systemctl --user disable --now bdag-fastsnap-seed.timer >/dev/null 2>&1 || true
  systemctl --user enable --now bdag-fastartifact-sidecar.timer
}
