...
- Recovery source: low-latency trusted peer on `192.168.49.186`.

## Adverse Conditions

1. The internal eMMC was too small for chain data, Docker churn, old chain backups,
...
2. Normal peer sync from an old local chain state was too slow for a practical
   recovery window.
3. A partial datadir could not be trusted as the final single source of
   node data after snapshot import.
...
3. Downloaded and verified the chain snapshot archive before import.
4. Parked old datadirs with timestamped names instead of deleting them, then
   imported the verified chain snapshot archive into a clean datadir.
